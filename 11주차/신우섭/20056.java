...
public class Main {
    ...
    static int n, m, k;
    static List<FireBall> fireBalls = new ArrayList<>();    // 존재하는 파이어볼 리스트
    static Map<Integer, int[]> ways = new HashMap<>();  // 방향 세팅
    ...
    private static void moveFireBall() {
        ...
        List<List<Integer>> fireBallIndexList = new ArrayList<>();  // 겹치는 파이어볼 저장할 리스트 
        ...
        for (FireBall fireBall : fireBalls) {
            ...
            int d = fireBall.d;
            int s = fireBall.s;
            int newS = s % n;   // 속력은 ⌊(합쳐진 파이어볼 속력의 합)/(합쳐진 파이어볼의 개수)⌋이다. int s에 mod 연산시 오류발생
    ...
    private static void mergeFireBalls(List<List<Integer>> fireBallIndexList) {
        List<Integer> removeList = new ArrayList<>();   // 삭제해야 할 파이어볼 리스트
